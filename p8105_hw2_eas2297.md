...
harbor_partial_data = left_join(mr_trash_wheel, precipm_2016)
```

    ## Joining, by = c("month", "year")

``` r
harbor_data = left_join(harbor_partial_data, precipm_2017)
```

    ## Joining, by = c("month", "year", "total")

``` r
rename(harbor_data, total_precip = total)
```
